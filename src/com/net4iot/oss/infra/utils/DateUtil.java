...
public class DateUtil {
	...
	public static String nowDate(SimpleDateFormat sdf) {
		...
		// 格式化当前时间
		String dayTime = sdf.format(dNow);
		return dayTime;
	}
	 ...
	 * @return
	 */
	public static String beforeDate(SimpleDateFormat sdf, int day) {
		Date dBefore = new Date();
		// 得到日历
		Calendar calendar = Calendar.getInstance();
		calendar.add(Calendar.DAY_OF_MONTH, day);
		dBefore = calendar.getTime();
		String dayTime = sdf.format(dBefore);
		return dayTime;
	}
	...
	public static void main(String[] args) {
		...
		Date now = new Date();
		System.out.println("当前天:"
				+ DateUtil.DateToString(now, DateUtil.LONG_DATE_FORMAT));
		System.out.println(tomorrow.after(now));
		System.out.println(addSecond("2013_12_31_23_59_59"));
		logger.debug(getAstro(birth));
	}

	/**
	 * 获取参数月的上 12个月的
	 * 起始时间,返回值是从最远的时间点开始，比如当前为2012年9月，那么则返回[2011/9,2011/10,....2012/8]
	 * 
	 * @param year
	 * @param month
	 * @return list[i][0] = start; list[i][1] = end; list[i][2] =
	 *         year+"/"+month;
	 */
	...
	/**
	 * 获取 在指定年份 的 指定月份之前的 月份 例如：2012年8月之前的月份 包含8月,最近的月份放在第一个element中
	 * 
	 * @param month
	 * @return list[i][0] = start; list[i][1] = end; list[i][2] =
	 *         year+"/"+month;
	...
	/**
	 * 包括beginYear年。 返回从beginYear年开始到去年的所有年份，最近的年份放在list的最开始处。
	...
	public static String addSecond(String strDate) {
		String[] arrStr = strDate.split("_");
		String result = "";
		if (arrStr != null) {
			Integer second = Integer.parseInt(arrStr[5]);
			Integer minite = Integer.parseInt(arrStr[4]);
			Integer hour = Integer.parseInt(arrStr[3]);
			Integer day = Integer.parseInt(arrStr[2]);
			Integer month = Integer.parseInt(arrStr[1]);
			...
			if (second.equals(59)) {
				...
				if (minite.equals(59)) {
					...
					if (hour.equals(23)) {
						arrStr[3] = "00";
						if (day.equals(getDaysOfMonth(arrStr[0], arrStr[1]))) {
							arrStr[2] = "01";
							if (month == 12) {
								arrStr[1] = "01";
								year += 1;
								arrStr[0] = year.toString();

							} else {
								month += 1;
								arrStr[1] = month.toString();
							}
						} else {
							day += 1;
							arrStr[2] = day.toString();
						}
					} else {
						hour += 1;
						arrStr[3] = hour.toString();
					}
				} else {
					minite += 1;
					arrStr[4] = minite.toString();
				}
			} else {
				second += 1;
				arrStr[5] = second.toString();
			}
			result = arrStr[0] + "_"
					+ StringUtil.addzero(Integer.parseInt(arrStr[1]), 2) + "_"
					+ StringUtil.addzero(Integer.parseInt(arrStr[2]), 2) + "_"
					+ StringUtil.addzero(Integer.parseInt(arrStr[3]), 2) + "_"
					+ StringUtil.addzero(Integer.parseInt(arrStr[4]), 2) + "_"
					+ StringUtil.addzero(Integer.parseInt(arrStr[5]), 2);
