package com.net4iot.oss.infra.orm;

import com.net4iot.oss.infra.orm.PropertyFilter.MatchType;
...
public class HibernateRepository {
	...
	public <T> Page<T> findPageSql(final Page<T> page, final Class entityClass,
	...
	public <T> Page<T> findPage(final Class entityClass, final Page<T> page,
		...
		if (page.isAutoCount()) {
			long totalCount = countCriteriaResult(c,page.getFirst()-1);
			page.setTotalCount(totalCount);
		}
	 ...
	 */
	protected long countCriteriaResult(final Criteria c,int firstResult) {
		...
		// 执行Count查询
		Long totalCountObject = (long) 0;
		// 执行Count查询
		if (0!=firstResult) {
			c.setFirstResult(0);
			totalCountObject = (Long) c.setProjection(Projections.rowCount())
					.uniqueResult();
	...
	protected Criterion[] buildCriterionByPropertyFilter(
		...
		for (PropertyFilter filter : filters) {
			if (!filter.hasMultiProperties()&&"companyId".equals(filter.getPropertyName())) {
				Criterion criterion = buildCriterion(filter.getPropertyName(),filter.getMatchValue(), filter.getMatchType());
				dis.add(criterion);
			}else if (!filter.hasMultiProperties()) { // 只有一个属性需要比较的情况.
