package com.net4iot.oss.domain.service;
...
public abstract class AbstractService {
	...
	public void batchSave(Object entityList) {
		hibernateRepository.batchSave((List<T>) entityList);
	}
	...
	public <T> Page<T> getPageSql(final Page<T> page, final Class classEntity,
			final String hql, final List<PropertyFilter> filters) {
		...
		for (int i = 0; i < filters.size(); i++) {
			...
			sb.append(" and ").append(pf.getPropertyName()).append(" ")
					.append(pf.getMatchType().getSql());
		...
		if (page.isOrderBySetted()) {
			...
			for (int i = 0; i < orderByArray.length; i++) {
				sb.append(" ").append(orderByArray[i]).append(" ").append(" ")
						.append(orderArray[i]);
		...
		return hibernateRepository.findPageSql(page, classEntity,
				sb.toString(), values);
	}
	...
	public <T> Page<T> findPageHql(final Page<T> page, final String hql,
		...
		for (int i = 0; i < filters.size(); i++) {
			PropertyFilter pf = filters.get(i);
			//过滤条件为公司Id时 -1也不过滤
			if(pf.getPropertyName().indexOf("companyId")!=-1){
				if(companyString.length()==0){
					companyString.append(" and ( ").append(pf.getPropertyName()).append(" ").append(pf.getMatchType().getSql());
				}else {
					companyString.append(" or ").append(pf.getPropertyName()).append(" ").append(pf.getMatchType().getSql());
				}
				values[i] = pf.getMatchValue();
			}else {
		...
		sb.append(companyString.append(")"));
		if (page.isOrderBySetted()) {
			String[] orderByArray = StringUtils.split(page.getOrderBy(), ',');
			String[] orderArray = StringUtils.split(page.getOrder(), ',');
			sb.append(" order by ");
	 ...
	 * @return
	 */
	public <T> List<T> findBySQLSimple(final String sql, final Object... values) {
		SQLQuery queryObject = getSession().createSQLQuery(sql);
    ...
    public <T> List<T> findBySQLtoBean(Class<T> clazz,final String sql, final Object... values) {
        SQLQuery queryObject = getSession().createSQLQuery(sql);
